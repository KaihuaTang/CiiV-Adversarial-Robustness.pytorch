...
class ResNet(Base_Model):
    ...
    def _forward_impl_all_sample(self, x, loop=10, bpda=False):
        ...
        for i in range(NUM_TOTAL_SAMPLE):
            if NUM_LOOP == 1:
                center_x = self.mask_center[1]
                center_y = self.mask_center[1]
            elif i % NUM_INNER_SAMPLE == 0:
            ...
            # attention
            mask = self.create_mask(w, h, center_x, center_y, alpha=10.0)
            sample = sample * mask.to(x.device)
