...
class ResNet(Base_Model):
    ...
    def _forward_impl_all_sample(self, x, loop=10, bpda=False):
        ...
        for _ in range(NUM_TOTAL_SAMPLE):
            on_sample = self.relu(x + x.detach().clone().uniform_(-1,1))
            #on_sample = self.relu(x + x.detach().clone().normal_(0,1))
            on_sample = on_sample / (on_sample + 1e-5)
            on_samples.append(on_sample)

            off_sample = self.relu(1.0 - x + x.detach().clone().uniform_(-1,1))
            #off_sample = self.relu(1.0 - x + x.detach().clone().normal_(0,1))
