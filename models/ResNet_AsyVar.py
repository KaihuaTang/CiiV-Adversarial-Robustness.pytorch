...
class ResNet(Base_Model):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, mask_center=[5, 16, 27], aug_weight=1.0,
                 groups=1, width_per_group=64, first_conv_size=7, num_sample=3, mask_aug=False, samp_aug=False,
        ...
        self.conv1 = nn.Conv2d(6, self.inplanes, kernel_size=first_conv_size, stride=2, padding=3,
                               bias=False)
    ...
    def _forward_impl_all_sample(self, x, loop=10, bpda=False):
        b, c, w, h = x.shape
        on_samples = []
        off_samples = []
        NUM_LOOP = loop
        ...
        for i in range(NUM_TOTAL_SAMPLE):
            if self.mask_aug:
                ...
                # attention
                masks.append(self.create_mask(w, h, center_x, center_y, alpha=10.0).to(x.device))
    ...
    def forward(self, x, loop=1, bpda=False):
        if x.shape[1] == 1:
            x = x.repeat(1, 3, 1, 1)
        if self.half_sample:
            raise ValueError('Half sample is not implemented!')
        else:
            return self._forward_impl_all_sample(x, loop=loop, bpda=bpda)
...
def create_model(m_type='resnet18', num_classes=1000, first_conv_size=7, num_sample=3, mask_aug=False, samp_aug=False, aug_weight=1.0, mask_center=[5, 16, 27]):
    # create various resnet models
    if m_type == 'resnet18':
        model = resnet18(num_classes=num_classes, first_conv_size=first_conv_size, num_sample=num_sample, 
                         mask_aug=mask_aug, samp_aug=samp_aug, aug_weight=aug_weight, mask_center=mask_center)
