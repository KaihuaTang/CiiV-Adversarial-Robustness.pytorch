...
class CW(Attacker):
    ...
    """
    def __init__(self, model, logger, config, c=1e-4, kappa=0, steps=1000, lr=0.01):
        super(CW, self).__init__("CW", model, logger, config)
        self.c = c
        self.kappa = kappa
        self.steps = steps
        self.lr = lr

        logger.info('Create Attacker CW with c: {}, kappa: {}, steps: {}, lr: {}'.format(c, kappa, steps, lr))
    ...
    def atanh(self, x):
        return 0.5*torch.log((1+x)/(1-x+1e-9))
    ...
    def f(self, outputs, labels):
        ...
        return torch.clamp(self.sign*(j-i), min=-self.kappa)
